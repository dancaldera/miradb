  ...
  return produce(state, draft => {
    switch (action.type) {
      ...
      case ActionType.SetError:
        draft.errorMessage = typeof action.error === 'string' ? action.error : action.error.message;
        draft.loading = false;
        break;
      ...
      case ActionType.UpdateSavedConnection: {
        const index = draft.savedConnections.findIndex(conn => conn.id === action.connection.id);
        if (index >= 0) {
      ...
      case ActionType.RemoveSavedConnection:
        draft.savedConnections = draft.savedConnections.filter(conn => conn.id !== action.connectionId);
        break;
      ...
      case ActionType.RemoveNotification:
        draft.notifications = draft.notifications.filter(note => note.id !== action.id);
        break;
