...
export function sortRows(rows: DataRow[], sortConfig: SortConfig): DataRow[] {
  ...
  return [...rows].sort((a, b) => {
    return compareValues(a[sortConfig.column!], b[sortConfig.column!], sortConfig.direction);
  });
}

export function filterRows(rows: DataRow[], filterValue: string, columns: ColumnInfo[]): DataRow[] {
  if (!filterValue.trim()) {
    return rows;
  }
...
export function processRows(rows: DataRow[], sortConfig: SortConfig, filterValue: string, columns: ColumnInfo[]): DataRow[] {
...
export function calculateColumnWidth(columnName: string, values: unknown[], maxWidth: number = 50): number {
