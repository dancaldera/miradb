...
import { z } from 'zod';
import type { ColumnInfo, ConnectionInfo, QueryHistoryItem, TableCacheEntry } from '../types/state.js';
...
async function ensureDataDirectory(): Promise<void> {
  try {
    ...
    throw new Error(`Failed to ensure data directory: ${(error as Error).message}`);
  }
}
...
export async function loadConnections(): Promise<ConnectionsLoadResult> {
  ...
  if (!Array.isArray(data)) {
    ...
    return { connections: [], normalized: 0, skipped: Array.isArray(data) ? 0 : 1 };
  }
...
export async function saveConnections(connections: ConnectionInfo[]): Promise<void> {
  await ensureDataDirectory();
...
export async function saveQueryHistory(history: QueryHistoryItem[]): Promise<void> {
  await ensureDataDirectory();
  const data = JSON.stringify(history, null, 2);
  await writeFile(historyPath, data, 'utf-8');
}

async function readTableCacheFile(): Promise<Record<string, Record<string, TableCacheEntry>>> {
  await ensureDataDirectory();
  ...
  for (const [connectionId, caches] of Object.entries(data as Record<string, unknown>)) {
    if (typeof caches !== 'object' || caches === null) {
      console.warn(`Skipping table cache for connection ${connectionId}: expected object.`);
      continue;
    }

    const connectionCache: Record<string, TableCacheEntry> = {};
    for (const [tableKey, entry] of Object.entries(caches as Record<string, unknown>)) {
...
export async function loadTableCache(connectionId: string): Promise<Record<string, TableCacheEntry>> {
  const file = await readTableCacheFile();
  return file[connectionId] ?? {};
}

export async function saveTableCache(connectionId: string, cache: Record<string, TableCacheEntry>): Promise<void> {
  const file = await readTableCacheFile();
...
function safeParse<T>(schema: z.ZodType<T>, data: unknown): T | null {
  ...
  console.warn('Failed to parse persisted data, using defaults:', result.error.flatten());
  return null;
}

function parseArray<T>(data: unknown, schema: z.ZodType<T>): T[] {
  if (!Array.isArray(data)) {
    console.warn('Expected array while parsing persisted data, using empty list.');
    return [];
  }
...
function normalizeConnectionEntry(entry: unknown): ConnectionWithMarker | null {
  ...
  const name = typeof record.name === 'string' ? record.name : 'Legacy connection';
  const driver = getLegacyString(record, ['driver', 'type']);
  const connectionString = getLegacyString(record, ['connection_str', 'connectionString']);
...
function getLegacyString(record: Record<string, unknown>, keys: string[]): string | undefined {
