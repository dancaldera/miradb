...
export const RowDetailView: React.FC = () => {
  ...
  useInput((input, key) => {
    ...
    if (editingField === null) {
      ...
      if (key.downArrow && state.columns && selectedField < state.columns.length - 1) {
        setSelectedField(selectedField + 1);
      }
  ...
  const handleEditSubmit = async () => {
    if (editingField === null || !currentRow || !state.activeConnection || !state.dbType) {
      return;
    }
  ...
  if (!state || !state.dataRows || state.dataRows.length === 0 || !state.columns || state.columns.length === 0) {
    return (
      <ViewBuilder
        title="Row Details"
        content={
          <Box flexDirection="column">
            ...
            <Text color="gray">Select a row from the data preview view first.</Text>
            {!state && <Text color="red">State is undefined</Text>}
            {state && !state.dataRows && <Text color="red">DataRows is undefined</Text>}
            {state && state.dataRows && state.dataRows.length === 0 && <Text color="yellow">DataRows is empty</Text>}
            {state && !state.columns && <Text color="red">Columns is undefined</Text>}
            {state && state.columns && state.columns.length === 0 && <Text color="yellow">Columns is empty</Text>}
          </Box>
        }
  ...
  const renderValue = (value: any, columnName: string) => {
    if (value === null || value === undefined) {
      return <Text color="gray" italic>NULL</Text>;
    }
    ...
    <ViewBuilder
      ...
      content={
        <Box flexDirection="column">
          <Box marginBottom={1}>
            <Text color="blue">
              Row Data ({state.columns.length} columns)
            </Text>
              ...
              >
                <Box flexDirection="row" justifyContent="space-between" paddingX={1}>
                  <Box flexDirection="row">
                    ...
                    {column.isPrimaryKey && (
                      <Text color="yellow" bold> [PK]</Text>
                    )}
                    {column.nullable && (
                      <Text color="gray"> nullable</Text>
                    )}
                  </Box>
                </Box>
