			...
			<Box flexDirection="column">
				<Box flexDirection="column" marginBottom={1}>
					...
					<Text color="gray" dimColor>
						{showCommands ? (
							<Text color="yellow">
								Command Mode: 'c' to clear, 'h' for history, Tab to exit
							</Text>
						) : (
				...
				{lastResult && (
					<Box flexDirection="column" marginBottom={1}>
						<Text color="green">
							✓ Last query executed successfully ({lastResult.rowCount} rows,{" "}
				...
				{(state.queryHistory?.length ?? 0) > 0 && (
					<Box flexDirection="column">
						<Text color="cyan">
							Query History: {state.queryHistory?.length ?? 0} quer
							{(state.queryHistory?.length ?? 0) === 1 ? "y" : "ies"}
						</Text>
						<Text color="gray" dimColor>
