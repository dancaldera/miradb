...
export const QueryHistoryView: React.FC = () => {
  ...
  const validHistory = rawHistory.filter(item => {
    return item && typeof item === 'object' && typeof item.query === 'string' && item.query.length > 0;
  });
  ...
  useEffect(() => {
    if (selectedIndex >= currentPageHistory.length && currentPageHistory.length > 0) {
      setSelectedIndex(0);
    }
    ...
    >
      {validHistory.length === 0 ? (
        ...
        <Box flexDirection="column">
          ...
          <Box marginBottom={1}>
            <Box marginBottom={1}>
              <Text color="blue" bold>Recent Queries:</Text>
            </Box>
            ...
            {totalPages > 1 && (
              <Box marginBottom={1}>
                <Text color="cyan">
                  Page {currentPage + 1} of {totalPages} (Queries {startIndex + 1}-{endIndex} of {validHistory.length})
                </Text>
              ...
              >
                ...
                <Box flexDirection="row" justifyContent="space-between">
                  <Box flexDirection="row">
                    <Text color={index === selectedIndex ? "cyan" : (item.error ? "red" : "green")}>
                      {item.error ? "✗" : "✓"}
                    </Text>
                ...
                >
                  {item.query.length > 100 ? item.query.substring(0, 97) + "..." : item.query}
                </Text>

                {/* Error Message */}
                {item.error && (
                  <Text color="red" dimColor>
                    Error: {item.error.length > 60 ? item.error.substring(0, 57) + "..." : item.error}
                  </Text>
                )}
              </Box>
            ))}
          ...
          {selectedQuery && (
            <Box flexDirection="column">
              ...
              <Box flexDirection="column" paddingX={1} borderStyle="single" borderColor="cyan">
                {/* Query Text */}
                <Box marginBottom={1}>
                  <Text color="white" wrap="wrap">{selectedQuery.query}</Text>
                </Box>
                ...
                <Box flexDirection="column">
                  ...
                  {selectedQuery.error && (
                    <Box marginTop={1}>
                      ...
                      <Text color="red" dimColor wrap="wrap">{selectedQuery.error}</Text>
                    </Box>
                  )}
                </Box>
              </Box>
