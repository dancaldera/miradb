...
import { connectToDatabase, removeSavedConnection, updateSavedConnection } from '../state/effects.js';
...
export const SavedConnectionsView: React.FC = () => {
  ...
  const [editMode, setEditMode] = useState<'name' | 'connectionString' | null>(null);
  ...
  const selectedConnection = items[selectedIndex]
    ? state.savedConnections.find(connection => connection.id === items[selectedIndex].value) ?? null
    : null;
  ...
  const cycleDbType = () => {
    ...
    void updateSavedConnection(dispatch, state, selectedConnection.id, { type: nextType });
  };
    ...
    <Box flexDirection="column">
      ...
      <Box marginTop={1}>
        {editMode && selectedConnection ? (
          <Box flexDirection="column">
            <Text>
              {editMode === 'name' ? 'Rename connection:' : 'Edit connection string:'}
            </Text>
            <TextInput
              value={inputValue}
              ...
              onSubmit={value => {
                ...
                setInputValue('');
                if (editMode === 'name') {
                  void updateSavedConnection(dispatch, state, selectedConnection.id, { name: trimmed });
                } else {
                  void updateSavedConnection(dispatch, state, selectedConnection.id, { connectionString: trimmed });
                }
              }}
            />
          ...
          <SelectInput
            ...
            onHighlight={item => {
              const idx = items.findIndex(candidate => candidate.value === item.value);
              if (idx >= 0) {
                setSelectedIndex(idx);
              }
            }}
            onSelect={item => {
              const connection = state.savedConnections.find(conn => conn.id === item.value);
      ...
      <Box marginTop={1}>
        <Text dimColor>
          Enter/o open • d delete • r rename • e edit string • t cycle DB type • Esc back
        </Text>
      </Box>
    </Box>
  );
