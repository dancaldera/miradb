...
describe('parameterize', () => {
  it('returns original SQL for PostgreSQL', () => {
    const query = parameterize('SELECT * FROM users WHERE id = $1', DBType.PostgreSQL, [1]);
  ...
  it('converts positional params to question marks for MySQL', () => {
    const query = parameterize('SELECT * FROM users WHERE id = $1 AND name = $2', DBType.MySQL, [1, 'Alice']);
  ...
  it('converts positional params to question marks for SQLite', () => {
    const query = parameterize('UPDATE users SET name = $1 WHERE id = $2', DBType.SQLite, ['Bob', 2]);
