...
vi.mock('../../src/utils/persistence.js', () => ({
  loadConnections: vi.fn(async () => ({ connections: [], normalized: 0, skipped: 0 })),
...
describe('effects', () => {
  ...
  it('initializeApp normalizes legacy connections and rewrites file', async () => {
    ...
    expect(persistence.saveConnections).toHaveBeenCalledWith([legacyConnection]);
    const actions = dispatch.mock.calls.map(call => call[0]);
    expect(actions).toEqual(expect.arrayContaining([
      expect.objectContaining({ type: ActionType.SetSavedConnections }),
      expect.objectContaining({ type: ActionType.AddNotification, notification: expect.objectContaining({ message: expect.stringContaining('Normalized') }) })
    ]));
  });
  ...
  it('fetchTables maps sqlite metadata to table info', async () => {
    ...
    vi.mocked(createDatabaseConnection).mockReturnValueOnce(sqliteConnectionStub as any);
    ...
    const actions = dispatch.mock.calls.map(call => call[0]);
    const setTablesAction = actions.find(action => action.type === ActionType.SetTables);
  ...
  it('fetchColumns maps postgres columns', async () => {
    ...
    vi.mocked(createDatabaseConnection).mockReturnValueOnce(columnsConnectionStub as any);
    ...
    await fetchColumns(dispatch, state, {
      type: DBType.PostgreSQL,
      connectionString: 'postgres://example'
    }, {
      name: 'users',
    ...
    const actions = dispatch.mock.calls.map(call => call[0]);
    const setColumnsAction = actions.find(action => action.type === ActionType.SetColumns);
    ...
    expect(actions).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ type: ActionType.SetRefreshTimestamp, key: 'public|users' })
    ]));
    expect(persistence.saveTableCache).toHaveBeenCalledWith('conn1', expect.any(Object));
  });
  ...
  it('fetchTableData appends rows and tracks pagination state', async () => {
    ...
    vi.mocked(createDatabaseConnection).mockReturnValueOnce(rowsConnectionStub as any);
    ...
    await fetchTableData(dispatch, state, {
      type: DBType.MySQL,
      connectionString: 'mysql://example'
    }, {
      name: 'users',
    ...
    const actions = dispatch.mock.calls.map(call => call[0]);
    const setRows = actions.find(action => action.type === ActionType.SetDataRows);
    const hasMore = actions.find(action => action.type === ActionType.SetHasMoreRows);
    const setOffset = actions.find(action => action.type === ActionType.SetCurrentOffset);
  ...
  it('fetchTableData sets non-zero offset when provided', async () => {
    ...
    const actions = dispatch.mock.calls.map(call => call[0]);
    const setOffset = actions.find(action => action.type === ActionType.SetCurrentOffset);
    expect(setOffset).toEqual({ type: ActionType.SetCurrentOffset, offset: 50 });
    expect(actions).toEqual(expect.arrayContaining([
      { type: ActionType.SetRefreshingTable, key: 'public|orders' },
      expect.objectContaining({ type: ActionType.SetRefreshTimestamp, key: 'public|orders' })
    ]));
    expect(persistence.saveTableCache).toHaveBeenCalledWith('conn2', expect.any(Object));
  });
  ...
  it('clearTableCacheEntry removes cache and refreshes data', async () => {
    ...
    await clearTableCacheEntry(dispatch, state, {
      type: DBType.PostgreSQL,
      connectionString: 'postgres://example'
    }, {
      name: 'users',
    ...
    const actions = dispatch.mock.calls.map(call => call[0]);
    expect(actions).toContainEqual({ type: ActionType.RemoveTableCacheEntry, key: 'public|users' });
    expect(persistence.saveTableCache).toHaveBeenCalledWith('conn3', {});
    expect(actions.some(action => action.type === ActionType.SetColumns)).toBe(true);
    expect(actions.some(action => action.type === ActionType.SetDataRows)).toBe(true);
    expect(actions.some(action => action.type === ActionType.AddNotification)).toBe(true);
  });
  ...
  it('throttles repeated table data refreshes', async () => {
    ...
    const actions = dispatch.mock.calls.map(call => call[0]);
    expect(actions).not.toContainEqual(expect.objectContaining({ type: ActionType.SetDataRows }));
    expect(actions).toContainEqual(expect.objectContaining({ type: ActionType.AddNotification }));
  });
  ...
  it('updateSavedConnection persists renamed connection', async () => {
    ...
    await effects.updateSavedConnection(dispatch, state, 'abc', { name: 'New Name' });

    const actions = dispatch.mock.calls.map(call => call[0]);
    expect(actions).toEqual(expect.arrayContaining([
      expect.objectContaining({ type: ActionType.UpdateSavedConnection, connection: expect.objectContaining({ name: 'New Name' }) }),
      expect.objectContaining({ type: ActionType.SetActiveConnection, connection: expect.objectContaining({ name: 'New Name' }) }),
      expect.objectContaining({ type: ActionType.AddNotification })
    ]));
    expect(persistence.saveConnections).toHaveBeenCalledWith(expect.arrayContaining([expect.objectContaining({ name: 'New Name' })]));
  });
  ...
  it('updateSavedConnection prevents duplicate names', async () => {
    ...
    await effects.updateSavedConnection(dispatch, state, 'def', { name: 'Prod' });
  ...
  it('updateSavedConnection requires non-empty connection string', async () => {
    ...
    await effects.updateSavedConnection(dispatch, state, 'abc', { connectionString: '   ' });
  ...
  it('updateSavedConnection refreshes active connection when connection string changes', async () => {
    ...
    await effects.updateSavedConnection(dispatch, state, 'abc', { connectionString: 'postgres://new' });

    const actions = dispatch.mock.calls.map(call => call[0]);
    expect(actions).toEqual(expect.arrayContaining([
      expect.objectContaining({ type: ActionType.SetActiveConnection, connection: expect.objectContaining({ connectionString: 'postgres://new' }) })
    ]));
    expect(actions).toEqual(expect.arrayContaining([
      expect.objectContaining({ type: ActionType.AddNotification, notification: expect.objectContaining({ message: 'Connection details changed; reconnecting…' }) })
    ]));
  });
  ...
  it('updateSavedConnection updates database type and reconnects active session', async () => {
    ...
    await effects.updateSavedConnection(dispatch, state, 'abc', { type: DBType.MySQL });

    const actions = dispatch.mock.calls.map(call => call[0]);
    expect(actions).toEqual(expect.arrayContaining([
      expect.objectContaining({ type: ActionType.SetActiveConnection, connection: expect.objectContaining({ type: DBType.MySQL }) }),
      expect.objectContaining({ type: ActionType.AddNotification, notification: expect.objectContaining({ message: 'Connection details changed; reconnecting…' }) })
    ]));
