...
describe('appReducer', () => {
  it('switches view and clears messages', () => {
    ...
    const result = appReducer(state, { type: ActionType.SetView, view: ViewState.Tables });
  ...
  it('sets db type and moves to connection view', () => {
    const result = appReducer(initialAppState, { type: ActionType.SelectDBType, dbType: DBType.PostgreSQL });
  ...
  it('adds query history items to the front', () => {
    ...
    const result = appReducer(initialAppState, { type: ActionType.AddQueryHistoryItem, item: historyItem });
  ...
  it('sets selected table and clears prior preview data', () => {
    ...
    const result = appReducer(state, { type: ActionType.SetSelectedTable, table });
  ...
  it('updates current offset via action', () => {
    ...
    const result = appReducer(state, { type: ActionType.SetCurrentOffset, offset: 100 });
  ...
  it('removes cache entry when requested', () => {
    ...
    const result = appReducer(state, { type: ActionType.RemoveTableCacheEntry, key: cacheKey });
